...
extern "C" void InitializeLogs() { absl::InitializeLog(); }
...
extern "C" void *UnsafeBufferPointer(PjRtBuffer *buffer) {
  auto unsafe =
      xla::ValueOrThrow(buffer->client()->UnsafeBufferPointer(buffer));
  return (void *)unsafe;
}
...
extern "C" xla::PjRtLoadedExecutable *ClientCompile(PjRtClient *client,
                                                    MlirModule cmod) {
  auto program =
      std::make_unique<xla::ifrt::XlaProgram>(cast<ModuleOp>(*unwrap(cmod)));
  ...
  auto exec = xla::ValueOrThrow(
      client->Compile(cast<ModuleOp>(*unwrap(cmod)), options));
  return exec.release();
}
